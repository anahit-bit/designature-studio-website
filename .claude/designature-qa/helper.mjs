 ...
 *   batch-fingerprint <slug> [--auth] [--save]   # reads routes from ./qa-config.yml (QA_ENV=production|staging|local)
 *   axe <url> [--auth]
...
async function newContext(headers, storageStatePath, viewportName = 'desktop') {
  const browser = await chromium.launch({
    headless: true,
    // QA_CHROME_PATH overrides the browser binary; otherwise Playwright's bundled Chromium.
...
async function batchFingerprint(slug, useAuth, save) {
  ...
  // QA_ENV selects environment.production (default), staging or local.
  const envName = process.env.QA_ENV || 'production';
  const baseRaw = config.environment?.[envName];
  if (!baseRaw) throw new Error(`qa-config.yml has no environment.${envName}`);
