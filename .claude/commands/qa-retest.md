You are running a targeted retest on designature.studio.
...
Output: qa-runs/reports/YYYY-MM-DD-retest-{ids}.md (and rendered .html)
...
4. Read .claude/designature-qa/PROFILES.md and qa-runs/findings-register.md — the register tells you where each finding was last confirmed.
...
Produce the report at qa-runs/reports/YYYY-MM-DD-retest-{ids}.md (replace {ids} with IDs separated by hyphens, e.g. "F5-F7-F11"). The retest report follows a simplified structure:
...
Then render HTML: `node .claude/designature-qa/render-report.cjs qa-runs/reports/YYYY-MM-DD-retest-{ids}.md`.
