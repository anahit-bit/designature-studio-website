You are running a **post-deploy regression check** on designature.studio. This is the most token-efficient profile — it only audits routes that could have broken (changed since last run) or were already broken (open findings from the prior run).
...
Output: qa-runs/reports/YYYY-MM-DD-deploy-check.md (and rendered .html)
Diff target: most recent canonical (or full-matrix) run.

SETUP

1. Read .claude/designature-qa/SKILL.md in full.
...
4. Read .claude/designature-qa/PROFILES.md and confirm the deploy-check profile spec matches what's above.
5. Confirm Playwright MCP is connected before proceeding. If not, stop and report what's missing.

DECIDE THE SCOPE

1. Run `node .claude/designature-qa/helper.mjs batch-fingerprint designature --save`. This reads every route from qa-config.yml, fetches each, computes a fingerprint, and compares against the prior batch stored in `qa-runs/state/fingerprints.json`. Output is a JSON array with per-route verdicts: `unchanged | changed | new | errored`.
2. Read the most recent prior report under `qa-runs/reports/`. Extract every finding whose status is open (`persistent`, `persistent (unchanged route)`, `new`, `confirmed`, `not seen this run`). Note their routes.
...
Produce the report at qa-runs/reports/YYYY-MM-DD-deploy-check.md following the skill's report structure:
...
Then render HTML: `node .claude/designature-qa/render-report.cjs qa-runs/reports/YYYY-MM-DD-deploy-check.md`.
