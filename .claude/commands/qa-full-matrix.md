You are running a QA pass on designature.studio.
...
Output: qa-runs/reports/YYYY-MM-DD-full-matrix.md (or qa-runs/reports/YYYY-MM-DD-full-matrix-part-N.md for split runs) (and rendered .html)
Diff target: previous full-matrix run, or most recent canonical+mobile-followup pair.

SETUP

1. Read .claude/designature-qa/SKILL.md in full. This is your playbook for method.
...
4. Read .claude/designature-qa/PROFILES.md and confirm the full-matrix profile spec matches what's above. Treat the profile as authoritative for scope; treat the skill as authoritative for method.
...
Produce the report at qa-runs/reports/YYYY-MM-DD-full-matrix.md (or part-N variant) following the skill's report structure:
...
Then render HTML: `node .claude/designature-qa/render-report.cjs qa-runs/reports/YYYY-MM-DD-full-matrix.md` (or the part-N variant).
