---
name: designature-qa
description: Use when the user asks to run a QA pass, test the site, or check production for regressions on designature.studio. Drives a real browser via the Playwright MCP server, visits each route in the test plan, interacts with key features, and produces a structured findings report (md + html). Do NOT use for writing Playwright spec files — that's a different task.
---
...
5. Output the findings report at the path defined by your invoking slash command (or by `PROFILES.md` if invoked manually). Filename patterns:
   - `qa-runs/qa-runs/reports/YYYY-MM-DD.md` for canonical
   - `qa-runs/qa-runs/reports/YYYY-MM-DD-smoke.md` for smoke
   - `qa-runs/qa-runs/reports/YYYY-MM-DD-locale-smoke.md` for locale-smoke
   - `qa-runs/qa-runs/reports/YYYY-MM-DD-mobile.md` for mobile-followup
   - `qa-runs/qa-runs/reports/YYYY-MM-DD-full-matrix.md` for full-matrix
   - `qa-runs/qa-runs/reports/YYYY-MM-DD-retest-{ids}.md` for retest (e.g. `retest-F5-F7`)
     Use the timezone in `qa-config.yml`'s `report_timezone:` field if set, otherwise UTC.
6. After writing the markdown, render an HTML version: `node .claude/designature-qa/render-report.cjs qa-runs/reports/<filename>.md`. The HTML lives next to the markdown and is what humans read.
...
6. Capture the build ID. Designature is a Vite SPA served by Express: the build ID is the hash in the main script tag, for example `/assets/index-Ab12Cd34.js` in the page HTML (`document.querySelector('script[type=module][src*="/assets/"]').src`). Log it in the report metadata. **If the site is built from multiple apps with independent deploy pipelines** (e.g. a marketing shell embedding a separately-deployed product app), capture each build ID separately — persistence of a finding can mean different things on different routes.
...
- Render HTML: `node .claude/designature-qa/render-report.cjs qa-runs/reports/<filename>.md`
