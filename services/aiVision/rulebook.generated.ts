 ...
 * being a rule.
 *
 * This build: 20 gemini rules · 0 staging · 9 not sent to any model
...
export const GEMINI_RULES: CompiledRule[] = [
  ...
  { id: "RD5", level: "ABSOLUTE", text: `Leave the ceiling as the single flat plane it already is, at its original height. Repaint it if the style calls for it and change the light fittings on it - but add nothing to it: no beams, soffits, bulkheads, coffers, dropped or tray sections, perimeter coves, shadow gaps, LED channels, floating panels, or plank, slat or timber cladding, and no downlights sunk INTO the plane. A light fitting hangs from the ceiling or sits on its surface; it is never recessed into it unless the photograph already shows recessed lights. If the photograph shows a plain flat ceiling, the renovated room has a plain flat ceiling - that is the correct result, not an unfinished one.` },
  ...
  { id: "RD24", level: "ABSOLUTE", text: `The room programme that follows lists what the finished room CONTAINS - furniture, fittings, lighting and styling. It can never authorise building work. If any part of it would need an opening, doorway, arch, staircase, window, wall, recess, change of level, or a NEW WATER SUPPLY OR WASTE PIPE that this photograph does not already show, DROP that part and furnish what is actually there. A room that reads as a dead end, or as small, or as awkwardly shaped, or as lacking a fixture you would expect it to have, is the correct answer when that is the room in the photograph.` },
  { id: "RD27", level: "ABSOLUTE", text: `Plumbed fixtures are fixed fabric, not furnishing. A toilet, bidet, bath, shower, basin or heated towel rail needs a waste pipe and a feed, so one may appear ONLY where the photograph already shows that fixture, or shows the boxing, duct or soil stack that serves it. Keep every plumbed fixture the photograph shows, on its own wall at its own size - replace or refinish it in the target style, but never move it to another wall and never delete it. Never add a plumbed fixture to a wall the photograph shows bare: floor space is not drainage. A bathroom photographed without a toilet is rendered without a toilet.` },
];
...
- [RD24] The room programme that follows lists what the finished room CONTAINS - furniture, fittings, lighting and styling. It can never authorise building work. If any part of it would need an opening, doorway, arch, staircase, window, wall, recess, change of level, or a NEW WATER SUPPLY OR WASTE PIPE that this photograph does not already show, DROP that part and furnish what is actually there. A room that reads as a dead end, or as small, or as awkwardly shaped, or as lacking a fixture you would expect it to have, is the correct answer when that is the room in the photograph.
- [RD27] Plumbed fixtures are fixed fabric, not furnishing. A toilet, bidet, bath, shower, basin or heated towel rail needs a waste pipe and a feed, so one may appear ONLY where the photograph already shows that fixture, or shows the boxing, duct or soil stack that serves it. Keep every plumbed fixture the photograph shows, on its own wall at its own size - replace or refinish it in the target style, but never move it to another wall and never delete it. Never add a plumbed fixture to a wall the photograph shows bare: floor space is not drainage. A bathroom photographed without a toilet is rendered without a toilet.`;

export const STAGING_RULES_BLOCK = ``;

/** The order the prompt's steps are written in. */
export type PromptSection = "clear" | "architecture" | "programme" | "furnish" | "finishes";
...
- [RD9] Joinery genuinely built into the room — a fitted kitchen run, fitted wardrobe, built-in banquette, reception or bar counter, fitted media wall — stays exactly where it is at exactly its current size. Refinish its surfaces in the target style, but never move, resize, extend, shorten or duplicate it.
- [RD11] If you cannot tell whether something is loose furniture or built into the room, treat it as loose and remove it.`,
  architecture: `- [RD1] Keep every window exactly as photographed — same wall, same position, same size, same shape, and the same number of panes and glazing bars. An arched window keeps its arch at the same height. Never add a window the photo does not show.
- [RD2] Keep every door and doorway exactly where it is, at the same width. A door you can see only part of — at the edge of the frame, or half hidden — is still a door and must still be there. Never add a doorway the photo does not show.
- [RD3] Keep every wall in its exact position and at its exact length, including short return walls, reveals and pier walls beside an entrance. If a wall is not visible in the photo, leave it out of frame — never invent one.
- [RD4] Photograph the room from the same spot with the same lens: identical proportions, ceiling height, camera angle and framing. Do not widen or deepen the room, and do not reveal more of it than the original frame shows.
- [RD5] Leave the ceiling as the single flat plane it already is, at its original height. Repaint it if the style calls for it and change the light fittings on it - but add nothing to it: no beams, soffits, bulkheads, coffers, dropped or tray sections, perimeter coves, shadow gaps, LED channels, floating panels, or plank, slat or timber cladding, and no downlights sunk INTO the plane. A light fitting hangs from the ceiling or sits on its surface; it is never recessed into it unless the photograph already shows recessed lights. If the photograph shows a plain flat ceiling, the renovated room has a plain flat ceiling - that is the correct result, not an unfinished one.
...
- [RD7] Restyle only what the photograph actually shows. If a wall, corner or room boundary is out of frame, leave it out of frame — never invent extra rooms, hallways, openings or enclosures.
- [RD27] Plumbed fixtures are fixed fabric, not furnishing. A toilet, bidet, bath, shower, basin or heated towel rail needs a waste pipe and a feed, so one may appear ONLY where the photograph already shows that fixture, or shows the boxing, duct or soil stack that serves it. Keep every plumbed fixture the photograph shows, on its own wall at its own size - replace or refinish it in the target style, but never move it to another wall and never delete it. Never add a plumbed fixture to a wall the photograph shows bare: floor space is not drainage. A bathroom photographed without a toilet is rendered without a toilet.`,
  programme: `- [RD24] The room programme that follows lists what the finished room CONTAINS - furniture, fittings, lighting and styling. It can never authorise building work. If any part of it would need an opening, doorway, arch, staircase, window, wall, recess, change of level, or a NEW WATER SUPPLY OR WASTE PIPE that this photograph does not already show, DROP that part and furnish what is actually there. A room that reads as a dead end, or as small, or as awkwardly shaped, or as lacking a fixture you would expect it to have, is the correct answer when that is the room in the photograph.`,
  furnish: `- [RD10] Finish the room completely, so it reads as lived in rather than as a showroom with a few pieces in it.
- [RD12] Keep every window completely clear. Place nothing tall in front of the glazing — no headboard, sofa back, cabinet, shelving or large plant — and let daylight through. Partly covering a window counts as covering it.
- [RD13] Keep doorways and the floor in front of them clear. Where a wall carries a door, choose a piece sized to the wall that is actually left over rather than one that covers the opening.
- [RD14] For clothes storage, first look for a recess, alcove or nook already built into a wall and fit the storage into it at that recess's existing width and depth. If the room has none, stand a freestanding armoire or wardrobe flat against a wall instead. Never carve a new recess into a flat wall.
- [RD15] Place the bed so it never covers a window or balcony door. If the only large wall in view carries the glazing, offset the bed to one side or stand it against a side wall, and never build a solid headboard wall across a window.
- [RD17] Render any screen, slatted panel or room divider as a freestanding piece of furniture, never as built-in architecture — and place it so it does not hide a door or window the photo shows.`,
  finishes: `- [RD16] Decorative cladding laid flat onto an existing wall — stone, tile, timber slats, panelling, wallpaper — is a finish and is welcome, as long as the wall stays in the same plane and nothing is cut into it.
- [RD18] Choose the light fittings deliberately and layer them — ambient, task and accent — in the target style. Fittings are yours to replace; the ceiling they hang from is not.`,
};
...
2. MATERIALS & FINISHES: layered and tactile, all of it current — velvet and mohair in jewel tones, ONE patterned wool rug with a bold contemporary or abstract design, printed linen and cotton in large-scale botanicals, wide stripes and modern geometrics, matte lacquered wood, blackened steel and matte brass, honed marble, glazed tile in a strong colour. Pattern meeting pattern is the point, and every pattern is a modern one.
3. FURNITURE CHARACTER: contemporary silhouettes in rich colour — a deep curved or modular sofa in jewel-tone velvet, two clean-lined armchairs that do not match each other, a low round table in lacquered wood or marble, a patterned ottoman, open shelving in slim steel. Legs are visible and refined, lines are clean. Exactly ONE piece may be genuinely vintage as a counterpoint; everything else is current. Keep silhouettes modern and let the colour and pattern do the talking.
...
6. DECOR & STYLING: abundance, carefully arranged and thoroughly contemporary — a dense gallery wall of modern abstract prints, graphic posters and contemporary photography in slim black, brass or coloured frames; art books stacked on every surface; ceramics and coloured glass grouped by hue; table lamps with pleated or printed shades; cushions in four or five modern prints; tall leafy plants; a few collected objects with character. Every surface holds something and nothing looks accidental.
7. OVERALL MOOD: rich, layered, personal and confident — and unmistakably a room of today. Depth and pattern rather than brightness, in a contemporary home whose owner collects and commits. Warm and enveloping, never a period drawing room.`,
...
export const ROOM_PROGRAM_RULES: Record<string, string> = {
  ...
  dining_room: `The room MUST be a fully realized DINING ROOM. The visual anchor is a dining table with 4-8 matching dining chairs, positioned as the centerpiece under a hanging pendant or chandelier - hung from the existing flat ceiling, never from a new coffer, tray, dropped section or bulkhead, and sized so the chairs pull out within the floor the photograph shows. Add a sideboard or credenza against one wall, wall art, and appropriate styling. Do NOT include lounge or living-room seating groups, lounge armchairs arranged around a low central table, media consoles, or televisions, even if the target style is often shown in a living room. Any style furniture examples below should be REINTERPRETED as dining-room equivalents - a dining chair in that style, a dining table in that style, a sideboard in that style.`,
  bedroom: `The room MUST be a fully realized BEDROOM. The visual anchor is a fully-made bed with a bedside table and lamp on each side WHERE THE WALL LENGTH ALLOWS - one side only, or a wall-mounted reading light, is the right answer in a narrow room, and is always better than moving the bed off its wall or shrinking anything around it. Add a bench at the foot, a dresser or wardrobe, a rug under the bed, art on the wall, and appropriate styling, each sized to the room the photograph shows. Do NOT include living-room furniture (sofa as primary piece, TV area), dining tables, or kitchen cabinetry. A reading chair in the corner is fine if the room is large. BED PLACEMENT (critical): every existing window or balcony door must stay fully visible, unobstructed, and exactly where it is - never cover it with the headboard, and never remove, shrink, move, or replace a window to make room for the bed. If the only large wall in view holds a window or balcony door, place the bed against a side wall or offset to one side of the window so the glazing stays completely clear; do not build a new solid headboard wall over a window, and do not hang art over a window.`,
  kitchen: `The room MUST be a fully realized KITCHEN. Include base and wall cabinetry, a countertop with backsplash, a range or cooktop, a sink with faucet, and the appliances the space takes (fridge, oven), plus open shelving or a hutch. Fit the run to the walls the photograph actually shows: cabinetry stands flush against them and stops where they stop. A range hood is an APPLIANCE - mount it on the existing wall or hang it from the existing flat ceiling, and never box it into a new bulkhead, soffit, chimney breast or dropped ceiling section. Add a kitchen island with counter stools ONLY where the visible floor area clearly leaves a walking route around it; in a narrow or single-wall kitchen, leave it out. Do NOT include living-room furniture (sofa, armchair, coffee table), bedroom furniture, or dining tables (unless a small breakfast nook is clearly the intent).`,
  bathroom: `The room MUST be a fully realized BATHROOM. It MUST be properly TILED: real wall tile behind the vanity and through the shower or bath area, and tiled or stone flooring, with visible grout lines and a tile format, colour and laying pattern chosen to suit the target style. Tile is a finish laid flat onto the walls that are already there - it never changes their plane. THE PLUMBING SURVEY ABOVE DECIDES WHICH FIXTURES EXIST, AND THIS PROGRAMME CANNOT ADD TO IT: keep every toilet, bidet, bath, shower and basin the photograph shows, each on its own wall at its own size, and add NONE that it does not. A bathroom photographed without a toilet is rendered without a toilet, and a wall shown bare stays bare - that is a correct result, not an incomplete one. Style what is genuinely there: replace or refinish the fixtures, the vanity, the mirror, the tiling, the lighting and the floor, and add the unplumbed things freely - mirror, storage, towel bars or rings with real towels, sconces or vanity lighting, a bath mat, plants and styling. UPGRADE THE FIXTURES THEMSELVES, do not merely repaint them: a bath becomes a new contemporary bath in the same alcove at the same footprint, with a tiled-in deck or built-in surround in the target style; a shower curtain becomes a fixed glass screen or frameless panel on the same bath edge; taps, the shower set, the waste and the overflow become new matching brassware in the style's metal; a basin gets a new basin and vanity in the same position; a toilet gets a new toilet in the same position. The drain and supply positions do not move; everything visible above them does. Do NOT carve a new recess, niche, wet-room enclosure or partition to house a fixture, do NOT enlarge the room to fit one, and do NOT add a heated towel rail unless the photograph already has one. Do NOT leave the walls as bare plaster or paint alone, and do NOT include living-room furniture, bedroom furniture, or dining tables. Every fixture must be a real bathroom fixture.`,
  home_office: `The room MUST be a fully realized HOME OFFICE. The visual anchor is a desk with a task chair. Add a task lamp, bookshelves or storage standing flush against an existing wall, monitor(s) or a laptop, wall art, and appropriate styling, each sized to the room the photograph shows. Do NOT include living-room furniture as the primary piece, beds, or dining tables. A small accent chair for a reading corner is fine where the floor area allows.`,
  kids_room: `The room MUST be a fully realized KIDS' BEDROOM or PLAYROOM. Include a child-scale bed, a small desk, storage bins or cubbies, a rug for floor play, playful wall art, and age-appropriate styling. Every piece is sized to the room in the photograph: use bunk beds only where the existing ceiling height clearly takes them, and never raise the ceiling, widen the room or borrow floor area to fit a piece in. Do NOT include adult living-room furniture, formal dining, or kitchen fixtures. BED PLACEMENT (critical): every existing window or balcony door must stay fully visible, unobstructed, and exactly where it is - never cover it with the headboard, and never remove, shrink, move, or replace a window to make room for the bed. If the only large wall in view holds a window or balcony door, place the bed against a side wall or offset to one side of the window so the glazing stays completely clear.`,
  outdoor: `The room MUST be a fully realized OUTDOOR SPACE (patio, terrace, or balcony as appropriate to the original photo). Include outdoor-rated seating (sofa, chairs, or dining set as fits the space), an outdoor rug, planters with real outdoor plants, string lights or outdoor sconces, and appropriate styling. All materials must be weather-appropriate. Keep the existing structure - railings, parapets, canopies, posts and the building wall behind - exactly as photographed; add no pergola, roof, screen wall or built structure the photo does not show. Do NOT include indoor furniture that would not survive weather.`,
  hallway: `The room MUST be a fully realized HALLWAY - a circulation space, furnished lightly, with the centre of the floor left clear enough to walk through. Furnish it as a hallway: a narrow console or hall table against one wall, wall art or a gallery arrangement, a runner rug following the length of the space, wall sconces or pendants, and a slim bench, hooks or shoe storage where a wall allows. THE THROUGH-VIEW IS THE PHOTOGRAPH'S TO GIVE, NOT YOURS TO CREATE. Where the photo already shows a doorway, opening, turn or staircase, keep it exactly as it is, clear and unobstructed, and let the eye travel to it. Where the photo does not show one, this hallway ENDS at the wall in front of you: do NOT cut an opening, arch, doorway or staircase into any wall, and do not suggest one with a recess, a painted arch or a lit alcove. A hallway that reads as a dead end is the correct result for a dead-end photograph. Do NOT include living-room seating groups, beds, or dining tables.`,
